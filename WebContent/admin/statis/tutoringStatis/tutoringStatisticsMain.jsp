...
.tutoringStatisticMain .table tbody tr:hover {
    cursor: pointer;
...
<br>
	...
	<div class="row rangebox float-right mt-3">		
		<div class="drop col-md-12 col-lg-12 col-sm-12 col-12">
	...
	<div class="container tutoringStatisticMain text-body">
		<jsp:include page="tutoringStatistics.jsp" />
	</div>
...
<%@include file = "../../../../footer2.jsp" %>
	...
	$('#all').click(function() {
		var rRrt = $.ajax({
			url : "starters?cmd=tutoringStatiscticsAll",
	...
	$('#aMonth').click(function() {
		var rRrt = $.ajax({
			url : "starters?cmd=tutoringStatiscticsOne",
			success : function(result) {
				console.log("1개월");
				$("div.tutoringStatisticMain").html(result);
			}
	...
	$('#threeMonth').click(function() {
		var rRrt = $.ajax({
			url : "starters?cmd=tutoringStatiscticsThree",
			success : function(result) {
				console.log("3개월");
				$("div.tutoringStatisticMain").html(result);
			}
	...
	$('#sixMonth').click(function() {
		var rRrt = $.ajax({
			url : "starters?cmd=tutoringStatiscticsSix",
			success : function(result) {
				$("div.tutoringStatisticMain").html(result);
			}
		});
		...
		if ( from == "" && to == "" ) {
			...
			$.ajax({
				url : "starters?cmd=tutoringDatesSelectAction&date="+from+to,
				success : function(result) {
					$("div.tutoringStatisticMain").html(result);
				}
			});
	...
	// 후기, 수입, 튜터링 횟수 올림차순 내림차순
	function abc(selectedguy) {
	var selectBox = selectedguy;
		$.ajax({
			url : "starters?cmd=tutoringSelectStatis&select=" + selectBox,
			success : function(result) {
				$("div.tutoringStatisticMain").html(result);
			}
		})
