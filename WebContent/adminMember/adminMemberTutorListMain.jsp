...
<title>스타터스 관리자 : 튜터회원 목록</title>
<div id="wrap">
<section>
	<article>
	<div class="container Main">
	<br>
		<h4 onclick="location.href='starters?cmd=adminMemberTutorList'" class="mb-5 text-body">튜터회원 목록</h4>
		
		<form class="col-12" method="post"
			action="starters?cmd=adminMemberTutorListAction">
		...
		<c:choose>
			<c:when test='${empty searchName}'>
					<jsp:include page="adminMemberTutorList.jsp" />
			</c:when>
			<c:otherwise>
					<jsp:include page="adminMemberTutorListSearch.jsp" />
			</c:otherwise>
		</c:choose>
				</div>
...
<script>
	...
	function abc(selectedguy) {
		...
		$.ajax({
			...
			url : "starters?cmd=adminMemberTutorListSelectAction&select=" + selectBox,
